...
async def listar_partidos(
    ...
    return await PartidoService(session).list(skip=skip, limit=limit, torneo_id=torneo_id, estado=estado)
...
async def actualizar_partido(
    partido_id: int, data: PartidoUpdate, session: AsyncSession = Depends(get_db)
) -> PartidoOut:
    return await PartidoService(session).update(partido_id, data)
