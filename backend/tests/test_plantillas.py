...
async def test_admin_da_de_alta_jugador_en_equipo(client: AsyncClient, admin_general_headers: dict[str, str]):
    resp = await client.post("/api/v1/jugadores", json={"nombre": "Fichaje Nuevo"}, headers=admin_general_headers)
...
async def test_dorsal_repetido_en_equipo_vigente_es_rechazado(
    ...
    resp = await client.post("/api/v1/jugadores", json={"nombre": "Otro Jugador"}, headers=admin_general_headers)
    jugador_id = resp.json()["id"]
