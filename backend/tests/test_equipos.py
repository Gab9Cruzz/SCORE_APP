...
async def test_admin_crea_actualiza_y_da_de_baja_equipo(client: AsyncClient, admin_headers: dict[str, str]):
    resp = await client.post("/api/v1/equipos", json={"nombre": "Nuevo FC"}, headers=admin_headers)
    assert resp.status_code == 201
    equipo_id = resp.json()["id"]

    resp = await client.patch(
        f"/api/v1/equipos/{equipo_id}", json={"nombre": "Nuevo FC Renombrado"}, headers=admin_headers
    )
    assert resp.status_code == 200
    assert resp.json()["nombre"] == "Nuevo FC Renombrado"

    resp = await client.delete(f"/api/v1/equipos/{equipo_id}", headers=admin_headers)
    assert resp.status_code == 200
