...
/** Partido es semi-custom (roles-3-modulos-plan.md, Fase 2, D4): el picker
 * de equipos depende del torneo elegido (solo equipos inscritos y no
 * cancelados — trg_partidos_validar_inscripcion en 06_triggers.sql
 * rechaza si no), y arbitro_id solo existe en PartidoUpdate, nunca en
 * PartidoCreate (Fase 1, D6) — por eso el form de creación y el de
 * edición son componentes distintos acá, no una config del scaffold
 * genérico. */
export function PartidosAdminPage() {
  ...
  if (modo.tipo === "crear") {
    return <CrearPartido onDone={volver} onCancel={volver} />;
  }

  if (modo.tipo === "editar") {
    return <EditarPartido fila={modo.fila} onDone={volver} onCancel={volver} />;
  }
      ...
      <ResourceTable<PartidoRow>
        ...
        columns={[
          { key: "torneo", label: "Torneo", render: (r) => nombreTorneo.get(r.torneo_id) ?? `#${r.torneo_id}` },
          {
            ...
            label: "Partido",
            render: (r) =>
              `${nombreEquipo.get(r.equipos_id_local) ?? "?"} vs ${nombreEquipo.get(r.equipos_id_visitante) ?? "?"}`,
          },
          {
            key: "fecha_partido",
            label: "Fecha",
            render: (r) => new Date(r.fecha_partido).toLocaleString("es-AR", { dateStyle: "short", timeStyle: "short" }),
          },
          { key: "estado", label: "Estado" },
        ]}
        isLoading={crud.listQuery.isLoading}
        isError={crud.listQuery.isError}
        emptyMessage="No hay partidos programados todavía."
...
function CrearPartido({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const inscritos = useResourceCrud<InscripcionRow>({
    ...
    listParams: torneoId !== null ? { torneo_id: torneoId } : undefined,
  ...
  // Solo equipos INSCRITOS (y no cancelados) en el torneo elegido — el
  // trigger del backend rechaza el partido si no, esto es UX, no la única
  // validación (ver diagrama en roles-3-modulos-plan.md, Fase 2, D4).
    ...
    <div className="page">
      ...
      <div className="resource-form">
        <label>
          ...
          >
            <option value="">{torneos.listQuery.isLoading ? "Cargando..." : "Elegir..."}</option>
        ...
        {torneoId !== null && (
          <>
            {!inscritos.listQuery.isLoading && equiposInscritos.length < 2 && (
              <p className="error-text">
                Este torneo tiene menos de 2 equipos inscritos — inscribí equipos antes de programar un partido.
              </p>
            )}
            <label>
              Equipo local
              <select
                ...
                disabled={inscritos.listQuery.isLoading}
              >
                <option value="">{inscritos.listQuery.isLoading ? "Cargando..." : "Elegir..."}</option>
...
function EditarPartido({ fila, onDone, onCancel }: { fila: PartidoRow; onDone: () => void; onCancel: () => void }) {
  const crud = useResourceCrud<PartidoRow>({ resourceKey: "partidos", basePath: "/api/v1/partidos" });
  // rol=Arbitro: server-side ya lo fuerza para TorneoAdmin (D5), y para
  // AdminGeneral (sin esa restricción) lo pedimos igual — el picker nunca
  // debe mostrar otra cosa que árbitros.
